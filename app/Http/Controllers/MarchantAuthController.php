...
class MarchantAuthController extends Controller
{

    public function signin()
    {
        return view('frontend.marchants.signin');
    }

    public function signup()
    {
        return view('frontend.marchants.signup');
    }
    ...
    public function signinPost(Request $request)
    {
        if($request->isMethod("POST")) {
            ...
            if ($this->attempt($request)) {
                //dd($request->all());
                return redirect()->route("marchant.dashbord");
            } else {
    ...
    private function attempt(Request $request)
    {
        ...
        $user = Admin::where(['email' => $email, "type" => "Admin"])->first();

        if ($user != null && Hash::check($password, $user->password)) {
            Session::put('marchant_id', $user->id);
            Session::put('marchant_name', $user->name);
            Session::put('marchant_email', $user->email);
    ...
    public function logout(Request $request){
        $email =  Session::get('marchant_email');
        DB::table("admins")->where('email',"=", $email)->update([
            'isLogin'=> false
        ]);
